    ...
    <div className="space-y-8">
      ...
      <section aria-label="Overview" className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {cards.map((c) => (
          <div key={c.label} className="rounded-2xl border border-border bg-card p-5">
            ...
            <p className="mt-2 text-3xl font-semibold">{isLoading ? "…" : c.value}</p>
          </div>
        ))}
      </section>
      ...
      {archived > 0 && (
        <p className="text-xs text-muted-foreground">
          {archived} archived character{archived === 1 ? "" : "s"}.
        </p>
      )}
      ...
      <section aria-label="Manage content" className="grid gap-3 sm:grid-cols-2">
        ...
        >
          <p className="text-xs uppercase tracking-widest text-muted-foreground">Factions</p>
          <p className="mt-1 text-lg font-semibold">Manage factions →</p>
          <p className="mt-2 text-sm text-muted-foreground">
            Search, create, edit, publish, and archive organizations.
          </p>
        </Link>
        <Link
          to="/admin/locations"
        ...
        >
          ...
          <p className="mt-1 text-lg font-semibold">Manage locations →</p>
          <p className="mt-2 text-sm text-muted-foreground">
            Manage location types, hierarchy, summaries, and publication.
          </p>
        </Link>
      </section>
    </div>
